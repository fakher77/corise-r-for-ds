...
tbl_names_popular_trendy = tbl_names |> 
  ...
  mutate(ratio = nb_births_max / nb_births_total) |> 
  # Group by sex
  group_by(sex) |> 
...
```
    ...
    #> # A tibble: 10 × 5
    #> # Groups:   sex [2]
    #>    sex   name     nb_births_total nb_births_max ratio
    #>    <chr> <chr>              <dbl>         <dbl> <dbl>
...
tbl_names_by_letter |> 
  ...
  # Initialize a ggplot of pct_births vs. first_letter
   ggplot(aes(x = first_letter , y = pct_births, fill = sex)) +
...
plot_trends_in_letter <- function(my_letter) {
  tbl_names_by_letter |> 
    ...
    labs(
      ...
      subtitle = glue::glue("Trends in sex by first letter {my_letter}"),
      caption = "Source: SSA",
      x = "Year",
      y = 'freqeuncy'
    ) +
...
How do these plots change when you focus on the `last_letter` instead of
the `first_letter`. Feel free to go back and change `first_letter` to
`last_letter` and see what you find. Try to make an educated guess
before you run the code, so you can see if your intuition matches up!

``` r
tbl_names_by_lletter = tbl_names |> 
...
plot_trends_in_lletter <- function(my_lletter) {
  tbl_names_by_lletter |> 
    # Filter for last_letter = my_letter
    filter(last_letter == my_lletter) |> 
    # Initialize a ggplot of pct_births vs. year colored by sex
    ggplot(aes(x = year , y = pct_births, color = sex))  +
    # Add a line layer
    geom_line() +
    # Add labels (title, subtitle, caption, x, y)
    labs(
      title = glue::glue("Trends in Names ending with {my_lletter}"),
      subtitle = glue::glue("Trends in sex by last letter {my_lletter}"),
      caption = "Source: SSA",
      x = "Year",
      y = 'freqeuncy'
    ) +
...
plot_trends_in_lletter("S")
```

<img src="img/BONUS-3_visualization-1.png" width="100%" style="display: block; margin: auto;" />
...
tbl_names_by_first_and_last_letter |> 
  ...
  labs(
      title = glue::glue("heatmap of the percentage of births by first letter and last letter"),
      subtitle = "Year 2021",
      x = "First letter",
      y = "Last letter",
      fill = "% Birth"
...
tbl_names_vowel_consonant |> 
  ...
  # Add labels (title, subtitle, caption, x, y)
  labs(
      title = glue::glue("The development of the pairing of initial and final letters in names over time"),
      subtitle = "Percentage across Year",
      caption = "SSA",
      x = "Year",
      y = "% Births",
      fill = "First/Last"  
